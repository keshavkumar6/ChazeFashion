...
def cart_view_session(request):
    cart = request.session.get('cart', {})
    products = Product.objects.filter(pr_id__in=cart.keys())
    cart_items = []
    total = 0
    for product in products:
        quantity = cart[str(product.pr_id)]
        subtotal = product.pr_price * quantity
        total += subtotal
        cart_items.append({
            'product': product,
            'quantity': quantity,
...
# --- UPDATE HOME TO SHOW ALL PRODUCTS ---
def home(request):
